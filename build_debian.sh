...
build_architecture() {
    ...
    # use work on every Debian suite we target.
    declare -a arr=("bookworm" "trixie" "forky" "sid")

    for dist in "${arr[@]}"; do
        FULL_VERSION="$rumdl_VERSION-${BUILD_VERSION}~${dist}_${build_arch}"
        echo "  Building $FULL_VERSION"

        if ! docker build . -t "rumdl-$dist-$build_arch" \
        ...
        id="$(docker create "rumdl-$dist-$build_arch")"
